...
int main(void) {
	...
	refresh();  // Atualiza a tela
		...
		int elapsed_time = (int)difftime(time(NULL), start_time);
                ...
                handleMovement(remaining_time);  // Função que lida com a movimentação recebe remaining time
	    ...
	    // Reseta as varíaveis para uma nova jogatina
            currentRoomIndex = 0;
            bonus = 0;
            steps = 0;
            strcpy(player_move, "");
...
// Inicia os eventos
void initializeEvents() {
    // Eventos do jogo, no momento usados somente para os segredos/bonus
    strcpy(events[0].description, "You found the secret!");
    events[0].effect = +10;
}

// Inicia as rooms com seus nomes, descrições e conexões predefinidas, algum dia faço um sistema melhor
void initializeRooms() {
